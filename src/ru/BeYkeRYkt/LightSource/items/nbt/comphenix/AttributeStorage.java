package ru.BeYkeRYkt.LightSource.items.nbt.comphenix;

import java.util.UUID;

import org.bukkit.inventory.ItemStack;

import ru.BeYkeRYkt.LightSource.items.nbt.comphenix.Attributes.Attribute;
import ru.BeYkeRYkt.LightSource.items.nbt.comphenix.Attributes.AttributeType;
import ru.BeYkeRYkt.LightSource.items.nbt.comphenix.Attributes.Operation;
...
public class AttributeStorage {
    ...
    public void setData(String data) {
        ...
        if (current == null) {
            attributes.add(Attribute.newBuilder().name(data).amount(0).uuid(uniqueKey).operation(Operation.ADD_NUMBER).type(AttributeType.GENERIC_ATTACK_DAMAGE).build());
        } else {
